...
import { useBisectionMethod } from '../hooks/useBisectionMethod';
...
const BisectionMethod = () => {
  ...
  const { calculate, loading, error } = useBisectionMethod();
  ...
  const handlePrevious = () => {
    const prev = previousCalculation('bisection');
    if (prev) {
      setCalculation(prev);
  ...
  const handleNext = () => {
    const next = nextCalculation('bisection');
    if (next) {
      setCalculation(next);
  ...
  return (
    <motion.div 
      className="method-page bisection-method"
      variants={pageVariants}
      initial="initial"
      animate="animate"
      exit="exit"
    >
      <Header title="Метод половинного ділення" showHomeButton={true} />
          ...
          {calculation && (
            <div className="download-section">
              <DownloadButton 
                ...
                method="bisection" 
                filename="bisection_calculation"
                graphRef={graphRef}
              />
            </div>
          )}
      ...
      <motion.div className="additional-links" variants={itemVariants}>
        <motion.div 
          ...
          whileTap={{ scale: 0.95 }}
        >
          <Link to="/videos/bisection" className="video-link">
        ...
        >
          <a 
          href="/assets/information/Teoretichni_Vidomosti_dla_mtyody_POLOVINNOGO_DILENNA.pdf" 
          target="_blank" 
          rel="noreferrer" 
